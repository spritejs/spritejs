...
(function () {
  ...
  var engine = Engine.create(); // engine.world.gravity.scale = 0; //turn off gravity (it's added back in later)

  var stackA = Composites.stack(100, 100, 6, 6, 0, 0, function (x, y) {
    return Bodies.rectangle(x, y, 15, 15, {// friction: 0,
      // frictionAir: 0,
      // frictionStatic: 0,
      // restitution: 1
    });
  ...
  var scene = new Scene('#simple-demo', {
    viewport: ['auto', 'auto'],
    resolution: [800, 600]
  });
  ...
  function render() {
    ...
    var bodies = Composite.allBodies(engine.world); // console.log(bodies)
...
(function () {
  var scene = new Scene('#render-demo', {
    viewport: ['auto', 'auto'],
    resolution: [800, 600]
  });
  var fglayer = scene.layer('fglayer');
