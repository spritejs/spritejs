...
(function () {
  var scene = new Scene('#coordinate', {
    viewport: ['auto', 'auto'],
    resolution: [1540, 600]
  });
  var layer = scene.layer();
  ...
  function createBox(x, size) {
    var box = new Label("".concat(size, "px"));
    var bgcolor = "rgb(".concat(size % 128 + 100, ", ").concat(size % 66, ", ").concat(size % 77, ")");
    box.attr({
      anchor: [0.5, 0],
      pos: [x, 100],
      size: [size, size],
...
(function () {
  ...
  anchorX.addEventListener('change', function (evt) {
    ...
    label.text = "anchorX: ".concat(value, ", anchorY: ").concat(y);
  });
  anchorY.addEventListener('change', function (evt) {
    ...
    label.text = "anchorX: ".concat(x, ", anchorY: ").concat(value);
  });
})();