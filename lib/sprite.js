...
import {removeUnit, rectToBox, boxToRect, boxUnion} from './utils'
  ...
  async render(t) {
    ...
    if(textures) {
      ...
      texturesWithImg.forEach(({texture, img}) => {
        ...
        if (texture.filter) {
            imgContext.filter = filters.toStr(texture.filter)
            if(texture.filter.dropShadow){
              ...
              const shadowRect = [Math.round(dsArr[0] - dsArr[2]), Math.round(dsArr[1] - dsArr[2]), imgRect[2] + 2 * dsArr[2], imgRect[3] + 2 * dsArr[2]]
              const finalRect = boxToRect(boxUnion(rectToBox(shadowRect), rectToBox(imgRect))).map(val => Math.abs(val))
            }
        }
        

        // if(texture.filter){
        //   const str = filters.toStr(texture.filter);
        //   imgContext.filter = str
        //   if(texture.filter.dropShadow){
        //     const dsArr = texture.filter.dropShadow.split(' ').map(value => removeUnit(value))
        //     const shadowRect = [Math.round(imgRect[0] + dsArr[0] - dsArr[2]), Math.round(imgRect[1] + dsArr[1] - dsArr[2]), imgRect[2] + 2 * dsArr[2], imgRect[3] + 2 * dsArr[2]]
        //     imgRect = boxToRect(boxUnion(rectToBox(shadowRect),rectToBox(imgRect))).map(value => Math.abs(value))
        //     imgCanvas.width = imgRect[2]
        //     imgCanvas.height = imgRect[3]
        //   }
