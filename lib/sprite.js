...
export class TextureAttr extends SpriteAttr {
  /*
    {
      src: ...,   //texture 路径
      srcRect: ...,  //texture 裁剪
  ...
  set textures(textures) {
    ...
    textures = textures.map(texture => {
      if(typeof texture === 'string') {
        texture = {src: texture}
      } else if(texture instanceof BaseSprite && texture.id == null) {
        texture.id = `texture${Math.random()}`
      }
      return texture
    })
...
class Sprite extends BaseSprite {
  ...
  get contentSize() {
    ...
    const boxSize = this[_texturesSize]
