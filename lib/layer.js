...
class Layer extends BaseNode {
  ...
  prepareRender() {
    if(!this[_state].prepareRender) {
      ...
      const that = this, _dispatchEvent = super.dispatchEvent
      requestAnimationFrame(async function step(t){
        ...
        if (animations.length) {
          ...
          that[_animations] = runningAnimations //保留未结束的动画
        }
        ...
        if(that[_updateSet].size){
          ...
          _dispatchEvent.call(that, 'update', 
            {target: this, timeline: that.timeline, currentTime: that.timeline.currentTime}, true)
