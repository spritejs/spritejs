...
import {boxIntersect, boxEqual, boxUnion, boxToViewRect} from './utils'
...
class Layer extends BaseNode {
  ...
  isVisible(sprite) {
    ...
    const maxWidth = this.outputContext.canvas.width,
      maxHeigth = this.outputContext.canvas.height
  ...
  //       //依次渲染，因为图片加载是异步的
  //       let {context, rect} = await child.render(t)
  //       shadowContext.drawImage(context.canvas, ...rect)
  //     }
  //   }
  ...
  async render(t) {
    ...
    const children = this[_children].filter(e => this.isVisible(e)) // 所有元素
    ...
    for(let i = 0; i < renderEls.length; i++) {
      const child = renderEls[i]
      if(child.parent === this) {
        const {context, rect} = await child.render(t)
        shadowContext.drawImage(context.canvas, ...rect)
      }
    }
