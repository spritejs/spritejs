  ...
  */
  querySelector(selector) {
    if(typeof selector === 'function') {
      ...
      throw new Error('unsupport select type')
    }
  }
  querySelectorAll(selector) {
    if(!selector || selector === '*'){
      ...
      switch(selector) {
        case 'sprite':
          return this[_children].filter(child => child instanceof Sprite)
        case 'label':
          return this[_children].filter(child => child instanceof Label)
        case 'path':
          return this[_children].filter(child => child instanceof Path)
        case 'axis':
          return this[_children].filter(child => child instanceof Axis)
        default:
          throw new Error('invalid sprite type')
      }
