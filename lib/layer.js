...
class Layer extends BaseNode {
  constructor(id, paper, {handleEvent, maxRenderPerFrame}) {
    super()
    ...
    this.maxRenderPerFrame = maxRenderPerFrame || Infinity
  ...
  async render(t) {
    ...
    const updateSet = this[_updateSet],
          maxRenderPerFrame = this.maxRenderPerFrame
    
    let updateEls

    if(updateSet.size <= maxRenderPerFrame){
      updateEls = [...this[_updateSet]] // 待刷新元素
      updateSet.clear()
    } else {
      updateEls = [...this[_updateSet]].slice(0, maxRenderPerFrame)
      updateEls.forEach(el => updateSet.delete(el))
    }
    ...
    for(const affectedEl of affectedEls) {
      if(updateSet.has(affectedEl)){
        updateSet.delete(affectedEl)
      }
      ...
      //因为 maxRenderPerFrame 存在，所以 affectedEl 也可能会有 lastRenderBox
      if(lastRenderBox && !boxEqual(lastRenderBox, box)){
  ...
  resolutionChange(){
    ...
    this.outputContext.canvas.width = width
    this.outputContext.canvas.height = height
    this.shadowContext.canvas.width = width
    this.shadowContext.canvas.height = height
