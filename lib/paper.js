...
class Paper {
  ...
  layer(id = 'default', handleEvent = true) {
    if(!this.hasLayer(id)) {
      const layer = new Layer(id, handleEvent)
  ...
  appendLayer(layer) {
    ...
    layer.zOrder = this[_zOrder]++

    sortLayer(this)   

    const _parent = Layer.symbolFor('parent')
    layer[_parent] = this
  ...
  removeLayer(layer) {
    ...
    if(this.hasLayer(layer)){
      const _parent = Layer.symbolFor('parent')
      layer.outputContext.canvas.remove()
      delete layer[_parent]
