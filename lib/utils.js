...
function parseValuesString(str, isInt = true) {
  if(typeof str === 'string'){
    let values = str.split(/[\s,]+/g)
    return values.map(v => isInt ? parseInt(v, 10) : parseFloat(v))
  } else {
    return str
  }
}
...
const getTextSize = memoize((text, font, lineHeight = 'auto') => {
