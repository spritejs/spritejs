...
class SpriteAttr {
  ...
  resetOffset() {
    ...
    if(offsetPath != null){
      const {x, y} = offsetPath.getPointAtLength(dis * offsetPath.getTotalLength())

      const offsetPoint = this[_attr].offsetPoint
      if(offsetPoint){
        this[_attr].transform.translate(-offsetPoint[0], -offsetPoint[1])
      }

      this[_attr].offsetPoint = [x, y]
      this[_attr].transform.translate(x, y)      
    }
  }
}
