...
const Resource = {
  loadTexture(texture, timeout = 30000) {
    ...
    let mapKey = texture.id || texture.src
    if(!loadedResources.has(mapKey)) {
      return new Promise((resolve, reject) => {
        if(typeof texture.src !== 'string'){ //允许用 sprites 做 texture
          if(!(texture.src instanceof BaseSprite)){
            const node = createNode(texture.src.nodeType)
            node.merge(texture.src.attr)
            node.id = texture.id
            texture.src = node
          }
